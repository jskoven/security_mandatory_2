...
def listen_for_other_patients():
    # Creating a socket for Alice, so that the other two patients can connect and send
    # messages to Alice.
    ...
    print("Alice is listening for the other patients")
...
def connect_to_other_patients():
    ...
    alice_secret = 40  # Example secret for Alice
    # Sample n-1 random shares
    s1 = random.randint(0, 100)
    s2 = random.randint(0, 100)
    # Compute Sn = S - (s1 + s2)
    s3 = alice_secret - (s1 + s2)
    assert s1 + s2 + s3 == alice_secret, "The sum of shares does not match the secret!"
    
    # Connect and send s1 to Bob
    ...
    with context.wrap_socket(client_socket, server_hostname="bob") as tls_sock:
        ...
        print(f"Alice connected to Bob.")

        message = f"From Alice, S1 = {s1}"
    ...
    with context.wrap_socket(client_socket, server_hostname="charlie") as tls_sock:
        ...
        print(f"Alice connected to Charlie.")

        message = f"From Alice, S2 = {s2}"
        tls_sock.send(message.encode('utf-8'))
